...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        clip_model_.cuda()
        self.dtype = clip_model_.dtype
        self.image_encoder = clip_model_.visual
    ...
    def forward(self):
        # print("image_feather:",image_features.shape)
        ctx = self.ctx
        if ctx.dim() == 2:
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        # print("text_dim",text_features.shape)
        score_kg = cos(text_features,text_features_old)
        score_kg = 1.0-torch.mean(score_kg)
        
        score_kd = self.hsic_loss(text_features,text_features_old)

        return logits, score_kg, score_kd
...
class DeKg(TrainerX):
    ...
    def build_model(self):
        ...
        self.w1 = cfg.TRAINER.COOP.W
        self.w2 = cfg.LOSS.WEIGHT
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            output,score_kg,score_kd = self.model(image)
            loss = F.cross_entropy(output, label)+self.w1*score_kg + self.w2*score_kd
