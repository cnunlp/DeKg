...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        text_features = self.text_encoder(prompts, class_prompt, 8.0,tokenized_prompts.detach()) 
...
class DeKg_TCP(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            input_test = torch.randn(1, 3, 224, 224).to(self.device)
            macs, params = thop.profile(self.model, inputs=(input_test,))
            print(f"Number of parameters: {params}")
            print(f"Number of MACs: {macs}")
            print('flops: %.2f M, params: %.2f M' % (macs / 1000000.0, params / 1000000.0))
